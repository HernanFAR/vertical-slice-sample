...
public class ExceptionHandlingStreamBehaviorExtensionsTests
{
    ...
    {
        protected internal override Eff<VSlicesRuntime, IAsyncEnumerable<TResult>> Process(Exception ex, TRequest request)
        {
    ...
    public class TestPipeline2<TRequest, TResult> : IStreamPipelineBehavior<TRequest, TResult>
        where TRequest : IStream<TResult>
    {
        public Eff<VSlicesRuntime, IAsyncEnumerable<TResult>> Define(TRequest request, Eff<VSlicesRuntime, IAsyncEnumerable<TResult>> next)
        {
